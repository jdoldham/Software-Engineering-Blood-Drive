...
function logIn(event) {
  $.ajax({
    type: 'POST',
    url: '/users/login/' + $('#logIn fieldset input#email').val() + '/' + $('#logIn fieldset input#pass').val()
  }).done(function( response ) {
    // Check for a successful (blank) response
    if (response.msg == true) {
      sessionStorage.setItem('email', $('#logIn fieldset input#email').val());
...
function addUser(event) {
  ...
  if(errorCount === 0) {
    ...
    var newUser = {
      ...
      'dob': $('#addUser fieldset input#inputDob').val(),
